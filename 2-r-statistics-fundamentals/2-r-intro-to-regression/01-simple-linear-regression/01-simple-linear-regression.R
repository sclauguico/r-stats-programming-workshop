...
ggplot(boston_real_estate_clean, aes(INDUS, TAX)) +
  geom_point(alpha = 0.5) +
  geom_smooth(method = "lm", se = FALSE)


# Run a linear regression of TAX vs. INDUS
lm(TAX ~ INDUS, data = boston_real_estate_clean)



# CATEGORICAL INDEPENDENT VARIABLES
boston_real_estate_clean <- boston_real_estate_clean %>%
...
ggplot(boston_real_estate_clean, aes(TAX)) +
...
summary_stats <- boston_real_estate_clean %>% 
  # Group by INDUS
...
mdl_tax_vs_INDUS <- lm(TAX ~ INDUS_group, data = boston_real_estate_clean)
