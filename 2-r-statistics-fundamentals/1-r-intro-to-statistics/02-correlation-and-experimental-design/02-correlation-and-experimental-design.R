...
# Based on the linear trend, what would most likely be the correlation?
...
# Correlation can be affected by range restrictions: If the range of values for one or both variables is limited, it can lead to an underestimated correlation coefficient. It's important to consider the potential impact of range restrictions on the interpretation of correlations.
...
# Remember, correlation is a valuable tool for understanding associations between variables, but it should be interpreted with caution, taking into account the specific context and potential limitations of the data and analysis methods.



# Create a scatterplot of mean sea level pressure vs. mean wind speed
ggplot(clean_weather, aes(Mean.Sea.Level.PressurehPa, Mean.Wind.SpeedKm.h)) +
  geom_point()

# Correlation between mean temperature vs. mean humidity
correlation <- cor(clean_weather$Mean.Sea.Level.PressurehPa, clean_weather$Mean.Wind.SpeedKm.h)
correlation
...
clean_weather <- clean_weather %>%
  mutate(log_mean_sea_level_pressure_hPa = log(Mean.Sea.Level.PressurehPa))

# Scatterplot of happiness_score vs. log_gdp_per_cap
ggplot(clean_weather, aes(log_mean_sea_level_pressure_hPa, Mean.Wind.SpeedKm.h)) +
  geom_point()

# Calculate correlation
cor(clean_weather$log_mean_sea_level_pressure_hPa, clean_weather$Mean.Wind.SpeedKm.h)
